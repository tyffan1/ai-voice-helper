...
from assistant.config import WAKE_NAME

_IDLE = "Слушаю..."
_RECORDING = "Записываю..."
_THINKING = "Думаю..."
_SPEAKING = "Говорю..."
...
def strip_wake(text, name=WAKE_NAME):
    import re
...
class Controller:
    ...
    def set_wake_enabled(self, enabled):
        ...
        print(f"Wake-отклик: {'включён' if enabled else 'выключен'}")
    ...
    def handle_text(self, text):
        ...
        if not text or len(text) < 3:
            self._speak("Не расслышал, повторите")
            return
        with self.busy:
            self.emit_log(f"Вы: {text}")
            self.emit_status(_THINKING, "#ff9800")
            decision = llm.ask(text)
            if "reply" in decision:
                self.emit_log(f"Атом: {decision['reply']}")
                self._speak(decision["reply"])
                return
            tool = decision.get("tool")
            params = decision.get("params") or {}
            self.emit_log(f"Действие: {tool} {params}")
            if tool == "exit_assistant":
                self.emit_log("Атом: До встречи")
                self._speak("До встречи")
                self.emit_status("Завершение...", "#f44336")
                if self.on_exit:
                    self.on_exit()
                else:
                    import os
            ...
            if result:
                self.emit_log(f"Атом: {result}")
                self._speak(result)
            else:
                self.emit_status(_IDLE, "#9e9e9e")

    def record_and_handle(self, source="горячая клавиша"):
        with self.busy:
            self.emit_status(_RECORDING, "#2196f3")
            print(f"[{source}] записываю...")
            ...
            print(f"[{source}] распознано: {text}")
